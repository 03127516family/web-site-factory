...
# product-summary（标题 + 快速参数 + 报价 CTA）。简介导语见正文 <!--block:summary-intro-->（多段，可编辑）
summary:
  specs:
    - "容量：3.2-80吨"
    - "跨度长度：4-31.5米"
    - "提升高度：根据客户的现场条件定制"
    - "工作职责。A5"
    - "工作电压：220V~690V，50-60Hz，3ph AC"
    - "保护等级。IP54 IP55"
    - "起重机控制模式。悬挂式控制/远程控制/机舱控制"
  cta: 报价要求sasa
...
## 优势 [!--block:advantages--](!--block:advantages--)
...
## 简介 [!--block:introduction--](!--block:introduction--)
...
## 简要技术参数比较 [!--block:spec-compare--](!--block:spec-compare--)
...
## 细节规格按要求提供 [!--block:spec-detail--](!--block:spec-detail--)
...
## 一种欧洲类型的桥式起重机将在45天内生产出来 [!--block:production-flow--](!--block:production-flow--)

来料检验 → 钢板开卷和开裂 → 钢板切割 → 滚动槽 → 焊接主大梁 → 起重机预装 → 欧洲型钢丝绳吊杆的组装 → 起重机包装和交付

> 提示。不同电压的起重机的交货时间可能会延长10-15天，因为电气元件需要由我们的供应商定制。

## 适用于不同工作条件的起重机类型 [!--block:crane-types--](!--block:crane-types--)
...
## 可进行现场安装或远程指导 [!--block:installation--](!--block:installation--)
