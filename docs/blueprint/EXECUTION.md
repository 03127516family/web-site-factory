...
| 1.3 | 图片闸门（**2026-07-04 重定范围，见决策 ㉕**）：图片存储既定走**外链**（模式2），压缩归 `media-ingest` 上传管道、**不建永久本地 `npm run img`**。当前先本地过渡：仅在**若本地批量搬迁前**对存量超标图（3–4MB 那批）做**一次性**压缩（可复用纯函数，未来即 media-ingest 核），带宽高属性 geom 不变。外链方案细节（MEDIA_BASE、不可变键、URL 指向）后续制定 | 存量图达标 + geom 1:1（仅在本地搬迁触发时才需） |
