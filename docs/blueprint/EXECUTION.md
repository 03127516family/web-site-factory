...
最后更新：2026-07-03　当前位置：**阶段 1 进行中——1.0 真相源收敛 ✅、1.1 SEO ✅、编辑链路全坐标探针实测 ✅、地基化第一步 ✅（引擎字符串入口 + 编辑链路回归门进 check，棘轮基线锁 27 已知缺陷）；下一个 = 1.2 面包屑 fragment，随后 1.8 编辑器缺陷修复 / E-3 引擎纯化**
...
| **1.8** | **编辑器写回缺陷修复（2026-07-03 全坐标探针实测发现，用户问"是否真能准确改任何地方"）**。455 坐标逐一真实写回验证：**428 过、identity 无损往返 302/302、重复组增删 36/36 全过**（含 gallery 19→20→18、specs/components/cases 等中间删项序列精确）。三处缺陷待修：① **name 写回黑洞（26 坐标）**——components/crane-types 双处结构里 `component.name` 坐标指向 `fm:components_images.N.name`，但渲染取正文 `###` 标题，**编辑组件名保存成功却不生效**；根治=按 §5 债务"收单处"（name 只留正文 ### 一处，坐标改指 ### 标题行）。② **`<p data-field="component.body">` 槽拆裂（超集模版 305 行等）**——列表型组件内容（如"保护装置"）注入 `<p>` 被拆到坐标外，浏览器里不可编辑；修法=该槽 `<p>`→`<div>`（§2.4 规则，改模版须 geom 复验）。③ **富文本存回 MD 漂移 10 处（视觉全部无损）**——`hero.highlights` 数组存回退化成 HTML 字符串（伤 ⑦ 字段级 diff 的粒度）、表格/图块往返格式规范化；修法=写回侧把 `<li>` 拆回 YAML 数组。探针脚本收编为 `scripts/edit-probe.mjs` 进 check（编辑链路从此有回归门） | 探针重跑 455/455 全绿 + geom 1:1 |
...
| E-4 | 富文本存回 MD 结构漂移（`hero.highlights` 数组→字符串等 10 处） | 1.8 已立项 | 写回侧保数组结构 | ⑦ 字段级 diff 粒度 |

**验收**：E-1/E-2 用一个"模拟两客户端并发写"脚本证并发拒绝；E-3 用"造一个假第二站 fixture、跑 build，引擎目录 diff 必须为空"证；E-4 归 1.8 探针。
