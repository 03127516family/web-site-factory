...
- 关键词调研、内容策略、外链——运营侧（7-01 决策）。
- hreflang / 多语言 SEO——随英文站触发；**正式设计已在 [12 章 §3.7](12-修改传播与多语言同步.md)**（hreflang+x-default 成对生成、每语言 sitemap/canonical、语言切换器同源、og:locale/inLanguage）。现在只保证 `/zh/` 路径结构不阻断它。
- 站长平台的持续监测运营——上线时提交一次 sitemap 即止，之后属运营。
