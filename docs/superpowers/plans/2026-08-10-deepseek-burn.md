...
9. **真 key 验收发现 #1（DeepSeek 的真实表达习惯）**：模型把「同字段跨多块」表达成每块一条目（`overview[2]/[5]/[6]`…），归类语义靠谱、形式违反「一字段一条目」——checkPlan 重复字段打回（T5.1 引入）三次重修仍不过，整次烧干净失败（防线按设计工作）。修：①checkPlan 同字段多条目**先合并再校验**（块并集无损；重叠仍抓；合并事件进 notes）——替代 T5.1 的重复打回；②提示词明确「同字段只许一条目」+ few-shot 示例改为多块形态；③**闭环**：阶段 2 改消费 `check.fields`（否则校验合并了、执行还按原表烧 3 次且 assemble 后写覆盖先写——实现者 DONE_WITH_CONCERNS 报出，控制器裁决补闭环）。回归钉 = T-merge 三条断言（72/72）。
10. **校验层收敛（2026-08-17，用户授权 AI 裁定后实裁；非缺陷修复，是策略调整）**：原则=「硬闸只留确定判定（子串有无/白名单/重叠>50%/schema），概率判定一律降级为提示」。四处：①裁 auditPositions 同段复用告警（结构性误报源——分不清「AI 把一段塞两格」与「原文本身重复」（散文+规格表同句是常态），修不好；重度重复 findDuplicates 硬闸管，轻度的人眼在 fieldMap 行自见）；②裁 assemble 期两次 validateDoc（plantField/assemblePost，与验收期 verifyTree 对同一确定性 mdToDoc 输出重复，同进程不可能失败；writeDraft 浏览器往返后那次保留——信任边界）；③「hero 图不烧」「trail 人工确认」两条 100% 开火的恒提示挪出 per-run 报告 → burn-console 静态常驻区（告警疲劳：每次都喊=没喊）；④相似级（section 标题/sections heading/text+similar 含 hero.headline）拒收→提示进 issues——核心理由=爆炸半径错配：标题级概率判定失败会把逐字满分的正文整格拖死。meta.md 的 level 语义同步（verbatim=硬闸/similar=提示/summary=豁免）。**已知风险（用户知情拍板）**：hero.headline 同降后，AI 组装营销语进 hero 无机器拦，靠人审预览兜底；反悔路径=单字段恢复硬闸。回归钉 = 5b 三条新断言（降级行为）+ 钉 5/13 改写（fieldMap 呈现）+ 钉 18 失败刺激源从假标题换成正文编造（128/128）。
