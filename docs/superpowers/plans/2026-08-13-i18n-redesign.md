...
import { SKIP_KEYS, skipPath, VALUE_SKIP } from './i18n-collect.mjs' // 同一套排除：键+路径前缀+值形态（值形态命中的串直通保留——它们永远不进 TM，删了就是生产镜像丢图，T3 质量审 C1）
...
// 段落走 applyInlineUnit 降序回植（质量审 C1：只拼接会丢句间分隔符——空格/hardBreak 属分隔符不进替换区，原生保留）；
// 句间空格合成（C1 合成半）：源无尾随分隔符 + 非 CJK 目标 + 后仍有保留句 → 补一个 ASCII 空格（full/approved 同行为，预览≡生产）。
function projNode(node, tm, mode, lang) {
  if (node.type === 'paragraph') {
    let nodes = node.content ?? []
    const { spans, text } = inlineSpans(nodes)
    const units = extractInlineUnits(nodes)
    const sepless = sp => !/[\s　]$/.test(text.slice(sp.start, sp.end))
    const cjk = /^(zh|ja|ko)/.test(lang ?? '')
    let keptAfter = false
    for (let si = units.length - 1; si >= 0; si--) { // 降序回植防 si 漂移
      const e = tm.sentences[fp(units[si].md)]
      if (mode === 'approved' && e?.status !== 'approved') { nodes = applyInlineUnit(nodes, si, ''); continue }
      ...
      const trail = keptAfter && sepless(spans[si]) && !cjk ? ' ' : ''
      nodes = applyInlineUnit(nodes, si, replacement, { annoMarks: anno, trail })
      keptAfter = true
    }
    if (mode === 'approved') { // 删句后悬空分隔符清边（段首孤 <br>/空白节点）
...
export function projectPage(srcJ, tm, mode, { lang, existingStatus, existingTrail } = {}) {
  if (mode !== 'full' && mode !== 'approved') throw new Error(`projectPage mode 非法: ${mode}`) // 拼错 mode 静默降级=中文占位数进生产，入口即抛（质量审 M4）
  ...
  const walk = (node, path) => {
    ...
    if (Array.isArray(node)) return node.map((v, i) => {
      if (typeof v === 'string') {
        ...
        if (mode === 'approved') return null // 未审串项剔除（中文不进生产；裸串挂不了 span 注解，pending 高亮只覆盖树句——有意简化）
        return e?.status === 'draft' ? e.translation : v
      }
      return walk(v, `${path}[${i}]`)
    }).filter(v => v !== null && !(v && typeof v === 'object' && !Array.isArray(v) && Object.keys(v).length === 0)) // null 项与空壳项都剔
    if (node.type === 'doc') {
    ...
    // 段一致性：有 body 的段，body 投影没了 → 整段删；任何带 title 对象标题未审 → 整体删（质量审 I2：推广到 inquiry_form/related_products 等，防空 h2/h3 残页）
    if (node.body?.type === 'doc' && out.body === undefined) return null
    if (typeof node.title === 'string' && out.title === undefined) return null
    return out
  }
  const result = walk(j, '')
...
export function loadTerms(srcLang, tgtLang) {
  ...
  return { lock: Array.isArray(j.lock) ? j.lock : [], map: j.map && typeof j.map === 'object' && !Array.isArray(j.map) ? j.map : {} }
}

// 校验：lock 去重非空；map 键值非空。烂词条拒收不落盘（schema 纪律同款）。
// 空白归一（质量审 Important）：先 trim 再校验再存——' HD ' 存进去会永远匹配不上（死词条）。
...
```js
// 机器验收（§5）：译文硬校验，不合格不落盘。六道：lock/map/数字/URL/CJK残留/覆盖率。
...
export function checkSentence(srcText, tgtMd, terms) {
  ...
  const normTerm = s => s.toLowerCase().replace(/-/g, ' ').replace(/\s+/g, ' ').trim() // map 表层归一：Title Case/连字符不卡自然行文（fails 仍报原形）
  ...
  if (/[㐀-鿿]/.test(tgtMd)) fails.push('cjk-residual') // 第六道：译文零中文残留（auto 语言兜底闸）
  const stripDates = s => s.replace(/\d{4}年\d{1,2}月(\d{1,2}日?)?/g, ' ') // 中文日期英译月份名属正确译法，源侧豁免
  const nums = s => (String(s).replace(/(\d),(\d{3})/g, '$1$2').match(/\d+(?:\.\d+)?/g) ?? []) // 千分位归一
  ...
  for (const u of (String(srcText).match(/https?:\/\/[^\s)\]。，；！？…」』）]+/g) ?? []).map(u => u.replace(/[.,;:!?]+$/, ''))) // 排 CJK 标点+尾随 ASCII 标点（非显然 WHY，注释留着）
    if (!tgtMd.includes(u)) fails.push(`url:${u}`)
  return { ok: fails.length === 0, fails }
}

// 覆盖率：送翻 id 全有译文。返回 missing 清单（空=齐）。畸形值（非字符串）不抛、记 missing。
export function checkCoverage(ids, translations) {
  const bad = id => typeof translations[id] !== 'string' || !translations[id].trim()
...
新建 `site/src/deepseek.mjs`：把 `deepseek-burn.mjs` 的 `createDeepseekCaller` 完整剪贴过来（含其上方注释块「DeepSeek 客户端（OpenAI 兼容…）」），`export` 保留。**两处对齐修订（T7 开工前实证现文件后计划方拍定）**：

1. **callAI 返回契约 = 解析后的 content 对象**（burn 全部消费方按此用：`const verdict = await callAI(msgs, tag)` 直接取字段）。i18n 引擎与 mock 都按这个契约来，不是 OpenAI 原始响应。
2. **noRetry 标志透传修复**：现文件 `if (e.noRetry) throw new Error(...)` 把标志包丢了——改为：
...
（实证：burn 无 `.noRetry` 外部消费方，此改动对 burn 零影响；grep 核过。）

`deepseek-burn.mjs` 删除该函数定义，替换为（规格审实证：纯 re-export 无本地绑定，burn() 内部 `callAI ??= createDeepseekCaller()` 会 ReferenceError——必须 import + export 两行）：

```js
// caller 已归位 src/deepseek.mjs（i18n 引擎复用）；import+re-export 保持既有 import 不破且本地可用
import { createDeepseekCaller } from '../src/deepseek.mjs'
export { createDeepseekCaller }
```

Run: `cd site && npm run accept:burn`
Expected: 全绿（证明挪动+noRetry 透传零行为变化）
...
export function translateMessages(batch, terms, lastFails = {}) {
  ...
  const retryNote = Object.keys(lastFails).length
    ? `\n\n上次这些 id 被机器验收拒收：${JSON.stringify(lastFails)}——逐条修正后重发。` // 温度 0 下不喂原因=确定性重放（烧制台 2026-08-11 真钱教训）
    : ''
  return [
...
export async function translateSegments(segments, terms, { callAI, batchSize = 30, maxRetries = 2 } = {}) {
  ...
  for (let off = 0; off < segments.length; off += batchSize) {
    ...
    for (let round = 0; round <= maxRetries && pending.length; round++) {
      const lastFails = Object.fromEntries(pending.map(s => [s.id, fail[s.id]]).filter(([, v]) => v)) // 上轮拒收原因（喂回提示词）
      let translations = {}
      try {
        const res = await callAI(translateMessages(pending, terms, lastFails), 'i18n')
        translations = res?.translations ?? {} // callAI 契约 = 解析后的 content 对象（与真 caller 一致）
      } catch (e) {
...
// 单元上下文：同字段内相邻句（前后各一）。注意：输入须是【未过滤的全量单元】——先挂上下文再滤送翻集，
...
export async function runPipeline(pageId, { lang = 'en', translate = true, callAI = null } = {}) {
  ...
  const missing = withContext(units).filter(u => !tm.sentences[u.fp]) // 先挂上下文再滤（质量审 M-4：before/after 须是真情邻居，不随送翻集缩水）
