...
- **2026-08-11** **烧制台两连改：选族 + 对照表退役（用户连续追问 + 4 评审并行收敛）**。①**选族**：控制台下拉「自动判断（默认）/产品页族/文章页族（待建灰）」；`burn()` 加 `family`，auto 时 AI 先判族（`classifyMessages`），判成未建族干净拒绝（不硬烧），显式选族跳过判族。②**对照表/块号规划层退役**：实战两次失败（无空行文章块号越界、发明近义字段名）全炸在规划层，且用户看不懂段号报告=人工闸门失效——改**一把梭**：AI 按内容理解一次出整页 JSON（`oneShotMessages`，max_tokens 8192），机器验收四道：防编（逐句规范化子串查全文）、**防错位**（`auditPositions` 命中位置机器算：同段复用+顺序颠倒告警，顺序只对正文段格否则误报）、防漏（`report.unused` 整段反查）、白名单；失败格单独重烧并**喂回拒收原因**（温度 0 下不喂就是确定性重放，重烧必败）；截断干净报错（自动逐格降级在 roadmap）。`checkPlan`/`planMessages`/merge 删除；**报告层纪律：内部机制词（段号/阶段/重试）永不上桌**，只给预览+对不上的原句+未用段落+fieldMap。验收 accept:burn 72 条全绿；geom 六页 1:1。详见对话记录 §六。
