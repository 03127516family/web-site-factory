...
- **2026-07-07** **i18n 翻译策略 U-1/U-2/U-3 拍定并落地（DECISIONS #7/#8/#9 从 OPEN 转 RESOLVED；引擎/渲染零改动，全落站点层+i18n 脚本）**。三条：
  ...
  - **U-2 术语表**与 U-1 行内保护词**合一**：`src/i18n/terms.<src>.<tgt>.yaml`（`lock:` 原样保留 / `map:` 中文→固定英文）。`i18n:apply --emit` 注入本页相关术语给翻译节点（AI）遵守；lint 做确定性后置校验，但**只报高置信两类**——`lock-lost`（锁词丢了）、`map-residual`（英文里残留中文）；**术语固定译法不做逐字硬校验**（`overhead crane`/`single-girder` 等合法大小写/连字符变体误报率过高，初版踩坑误报 81 次），只在 emit 提示层用。独立门禁 `npm run i18n:lint`（违规退出码 1）。
