...
- **2026-08-24** **素材真身归 site/（site 自包含拆雷，用户拍方案 A「反转」）**。背景：排查发现 site/ 对根目录 5 处活依赖，最大一颗是素材库——`public/assets`(48MB)+favicon 真身在根，site 经 `site/public→../public`、`dist/assets→../../public/assets` 两层软链取用；另有 img-probe/burn-lib/link-assets/accept-burn/accept-poc5 五处 `'../public'` 路径引用与 `scripts/lan.mjs` import 雷（已先拷贝解掉）。拍定**反转**：真身 `git mv` 进 `site/public/`（历史保留），根 `public/` 留真目录、内放 `assets`/`favicon.ico` 两软链回指——**根 public 整体软链不可行（实验证实：Node `fs.cp` 源根为软链直接抛 `ERR_FS_CP_NON_DIR_TO_DIR`；源内子目录软链在默认 `dereference:false` 下会以绝对路径软链进产物、换机即断）**，故根 `build.mjs` 的 cp 补 `dereference: true`（保持实拷行为不变，产物仍自包含）。site 侧五处路径改内；`img-probe.probe` 把相对 `dir` 锚到 `SITE`（免 cwd 歧义）；烧制验收 fixture 入 `site/content/raw/`；edit-server 保存链本就自跑 link-assets，指向自动维持。验证：双系统 build 绿（根 dist/assets 真身、site dist/assets 软链直连 `../public/assets`）、8092 页/css/img/favicon 全 200、accept:burn 155/155、accept:poc5 20/20（首跑 `#edlToggle` 偶发超时系 edit-layer UI 段 flake，重跑全绿；其测试污染——en 镜像机翻+TM 4 句+`.bak`——已 git checkout 还原）。**旧系统下线清单相应缩短**：剩 geom 基准（根 dist 存档）、首页/搜索/sitemap（R46/R47 待建）、切换本身。

- **2026-08-25** **site/src 按功能族收拢（用户发起；纯机械重构，零行为变化，单 commit）**。平铺 30+ 模块按族归文件夹：`src/i18n/`（kernel=原 i18n.mjs，pipeline/collect/sent/engine/checks/terms/tm/project/events，`data/` 子目录=TM/术语表/config）、`src/writeback/`（core/request/migrate）、`src/content/`（schema/revision/diff/source）、`src/draft/`（store/workflow）、`src/render/`（render=原 render-doc、tree-utils、mdast-tree）。留在 src 根：deepseek/asset-config/burn-lib/build-outputs/edit-context/edit-contract。**三类引用要一起改**（这次全踩过）：①相对 import（58+36 处，注意被挪文件内部的 `./邻居.mjs` 按老基座解析）；②`@src/` 别名 import（astro.config 别名，pages 与组件 8 处）；③字符串路径（TM/terms/config 的 join 路径、验收脚本里的落盘断言）。`.i18n-events.jsonl` 仍在 site 根。验证矩阵全绿：accept-i18n2 84/84、burn 155/155、writeback 34/34、content-diff 15/15、drafts 11/11、poc5 56/56（8092 端到端）、node22 build 15 页。**此后新模块一律按族落文件夹**（SEO 落 `src/seo/`）；决策日志更早条目里的老路径以本条映射为准。
- **2026-08-26** **SEO 模块落地（spec 2026-08-25 定稿，方案 A 构建流水线内现算；计划 docs/superpowers/plans/2026-08-26-seo-module.md，11 任务内联 TDD 执行完）**。`src/seo/`（config 纯常量 + kernel 纯函数）产出：每页 head 片段 `seoHead`（canonical/hreflang+x-default/og 兜底链/JSON-LD Product|Article+BreadcrumbList/noindex）贴 document.html `{{SEO}}` 槽（Chrome.astro 接线，与语言切换器同源同位置）；`scripts/seo-emit.mjs` build 后写 sitemap（全员 xhtml:link 互认、无 lastmod）+ robots.txt 进 dist/dist-edit 双产物链；`/__seo/health` 体检端点（长度/缺失/noindex + pin 待确认）。**两个实施修正**：`<title>`/`<meta description>` 仍由 `{{TITLE}}`/`{{DESCRIPTION}}` 发（seoHead 不重复发）；预览 noindex 复用现成 `INCLUDE_DRAFTS` 信号。**顺手修两处旧账**：①D1 前缀翻转 `SITE_BASE=SITE_ROOT`（中文住根——旧 `/zh/` 值与 dist 拓扑本就不一致，切换器 zh 链接 404）；②adoptMirror 补线进 /__save 镜像发布分支（此前全站无生产调用方，镜像人改不进 TM、下次重投影整文件覆盖冲掉人改——2026-08-18「改完存 TM 永不再犯」实际未接线）。**pin 机制（D5 锁定层）**：TM 新增 `pins` 字段账本（loadTm 同步保留该键），键=`pageId::field`；runPipeline 载入后复植（人稿按当前 fp 注入 approved/human，不送机翻）；`decidePins`+`/__i18n/pin-decide` 批量 keep（清旗保人稿）/refollow（删 pin 放行重翻）。**e2e 实证教训（已修）**：TM 是全语言对共享的，pin 只按字段名做键会一页收养六页污染——句账 fp 全局去重本身正确，pin 是页级人决策必须带 pageId。**翻译长度预算**：`page.title/description/seo.og.*` 四字段 budget（60/160）进翻译提示词（提示级非硬闸，2026-08-17 doctrine），治存量 8 页英文超长（6 标题+8 简介）。首次镜像发布会把存量手工调过、从未进 TM 的字段收养成人稿+pin（预期的一次性欠账收割，单梁 breadcrumb.current 即此例）。管理界面（单页面板/体检台 UI）另立计划，本期只保数据接口。验证矩阵：seo 16/16、check 84/84、burn 155/155、writeback 34/34、poc5 56/56、node22 build 15 页、e2e 四步真实链（收养→源改→旗亮人稿顶住→keep 清旗）全过。
- **2026-08-26（晚）** **工作台做实转正（spec 2026-08-26-workbench-design，方向 A；用户拍定单界面收编：「以后只有一个界面就是目前这个 admin 中的，其他的都不要」）**。design/admin-ui 三件套复制转正 `site/workbench/`（原目录留档）；`npm run workbench` 一键起 8090+8092（**须 node ≥22**——8092 的 astro build 在 node20 下退出码 1）。**数据层正源化**：读=直算 import 正源（`i18n/kernel.scanPages` 门禁口径、`loadTm/loadTerms/loadConfig`、`seo/kernel.healthData`、`pipeline.pinQueue/consoleData`——修掉 8-25 重构漏改的 TM 断路径[病根=隔离侧自带抄本]，`pagesData` 行带全语言覆盖 `langs`）；写=照旧全代理 8092（POST_PROXY 11 条，新增 /api/pin-decide）。**SEO 体检屏**（抄 Yoast Overview）：一行=一逻辑页、默认只显异常、行展开语言明细+pin 单条/批量保持与重跟（refollow 带花费确认弹层）、概览汇总卡、侧栏徽章。**退役**：烧制台 `/__burn`、翻译台 `/__i18n`（含审阅页 `/__i18n/review`、`review-data`）四条 HTML 路由删、burn-console/i18n-console 两文件删——POST/数据端点全保留（工作台代理依赖）；改句子=镜像页就地改+adoptMirror 收养，failed=工作台重送。**占位清零**：术语行内编辑/删除接 terms-save（prompt 弹层）、全部编辑/搜索/镜像链接直连 `EDIT_BASE`（删 editor.html 跳板与 s-editor 设计稿屏）、设置屏两处过时文案更正、镜像行「复查」死链删。编辑层 `workbenchUrl` 默认按访问 host 端口置换指 8090。验收 `accept-workbench` 7/7（自起服务断言 API 与正源逐项一致）+ 全矩阵回归（check 84/84、seo 16/16、burn 155/155、writeback 34/34、poc5 56/56、build 15 页）。**已知偏差**：非法 lang 打 pin-decide 回 500 非 400——edit-server 文件级约定普通 Error→500，与兄弟 i18n 端点一致，不改。**残留**：媒体库/搜索仍 server 自带实现（无正源模块）；询盘收件箱读 mock 后端（真后端未定）；工作台无鉴权（本机/局域网定位，上公网前必须加）。**此后管理界面只此一个**——新管理功能一律长在工作台里（新屏），不再建独立控制台。
- **2026-08-26（深夜）** **单页 SEO 编辑落地 + spec 错误声明更正**。SEO 体检屏展开行加 Yoast metabox 同构编辑（标题/简介输入+长度计数+谷歌预览+保存发布）；`/api/seo-save` 读 workspace revision 组 changes 代理 `/__save`；两族 edit-contract 注册 page.title/page.description（text，标题 required——length 仍提示级不硬拦）。e2e 实证：改英文镜像→JSON 更新+双字段收养+pin；改中文源→后台流水线重翻、人稿顶住、待确认旗亮。**更正**：workbench spec §4 原写「page.title/description 已是 8092 页面编辑器的可编辑字段」不属实（契约未注册、head 字段视觉编辑器够不着）——本次以 SEO 屏面板兑现，spec 已改。仍后补：og 覆盖/canonical/noindex 的编辑 UI（数据模型与渲染已就绪，触发条件=真有社交卡/收录需求时）。验证：poc5 56/56（契约变更回归）、accept-workbench 7/7、check 84/84、seo 16/16、UI 截图实证。**同晚补 Yoast 落位**（用户拍「和 wordpress 一样」）：8092 页面编辑器顶栏加 SEO 按钮 → 弹层（Yoast metabox 同构：谷歌预览点击聚焦标题/标题简介输入/长度计数）；head 字段无 DOM 坐标 → edit-layer 新增第四条 dirty 通道 `dirty.meta`（targetId→value 直存），改动进变更集随同一「发布」链保存（diff 弹层逐字显 page.title 新旧值）；保存/丢弃两处 dirty 清理点同步 meta.clear()。e2e 实证：面板改标题→dirty 计数→发布→diff 含 page.title→JSON 落盘；poc5 56/56。**至此双落位齐备**：日常编辑顺手改=8092 编辑页 SEO 面板（Yoast metabox 位）；治理异常=工作台体检屏展开行（Yoast Overview 位）。
